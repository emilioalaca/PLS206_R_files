...
bfat<-read.csv("~/Documents/TeachServ/CLASSES/AGR206/PLS206F13/Examples/bodyfatb.dat", header=T)
bfat <- cbind(scale(bfat[,1:3]),bfat)
...
identical(all.equal(pcaX$x, pcaZ$x), TRUE) # the PC scores are the same
...
sum(bf.pca$sdev^2) # each column of X results in a column in Z and each column in Z has a variance of 1, so the sum of the vraiances is 3
...
bfat <- cbind(bfat, pcaZ$x)
str(bfat)
m2<-lm(y ~ PC1 + PC2 + PC3, bfat) 
vif(m2)
summary(m2)

mean(bfat$y) # note that when using Z, the intercept is Ybar and it is the same, whether using Z or PC scores are predictors.
