...
#You have to understand that R as any language has a structure, syntax and a grammar (basically). In R we have packages, which contain functions that perform routines. In order to do that you have to load a package (or also called library) and after that apply the function on an object (let's say a data set).
...
filaree<-read.csv("~/Desktop/xmpl_filaree.csv", header=TRUE)
...
mean(C)
########## basic indexing #######
vals <- c(mean(C), mean(D), 1, 3, 5)
x <- letters
words <- c("never","give","up")
x[12]    # 12th element
words[2] # second element
words[5] # what happens when things are not found
vals[2]     # third element
